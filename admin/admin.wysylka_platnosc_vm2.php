...
if (isset($_POST['zapisz'])) {
    ...
    if (version_compare(JVERSION, '1.6.0', 'ge')) {
        $query = " UPDATE #__extensions SET params='" . $params->toString() . "' WHERE element='com_wysylka_platnosc_vm2' ";
    } else {
        $query = " UPDATE #__components SET params='" . $params->toString() . "' WHERE `option`='com_wysylka_platnosc_vm2' ";
    }
    $db->setQuery($query);
    ...
    $jap->redirect('index.php?option=com_wysylka_platnosc_vm2');
}
...
$db =& JFactory::getDBO();
if (version_compare(JVERSION, '1.6.0', 'ge')) {
    $query = 'SELECT extension_id FROM #__extensions WHERE element="wysylka_platnosc_vm2" AND enabled=1 ';
} else {
    $query = 'SELECT id FROM #__plugins WHERE element="wysylka_platnosc_vm2" AND published=1 ';
}
$db->setQuery($query);
...
?>
    ...
    <input type='hidden' name='option' value='com_wysylka_platnosc_vm2'>
</form>
